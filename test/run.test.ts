...
test("settings are effective, recorded, and final disk result matches return value", async () => {
  ...
  assert.equal(output.result.settings.maxLatencySamples, 2_000_000); assert.equal(output.result.settings.maxErrorExamples, 100); assert.equal(output.result.settings.measuredRequestTimeoutMs, output.result.config.timeoutMs); assert.equal(output.result.settings.sessionPreparationConcurrency, 10); assert.equal(output.result.settings.boundarySessionsUnmeasured, true);
  assert.equal(output.result.settings.executionTopology, "co-located");
  assert.deepEqual(output.result.settings.capacityRefinement, { method: "bounded-binary-search", maxStages: 4, minUserGap: 1 });
  assert.equal(resourceOptions.intervalMs, output.result.settings.resourceIntervalMs); assert.equal(resourceOptions.maxSamples, output.result.settings.resourceMaxSamples.value);
});
