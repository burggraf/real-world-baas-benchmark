...
Node runs asynchronous closed-model virtual users. Before measurement, ordinary-user sessions are prepared in deterministic input order in fixed batches of ten with no retries; preparation and final boundary cleanup are unmeasured and any preparation failure invalidates the stage. Each prepared virtual user owns an authenticated session and waits for an action to finish before thinking and starting the next action. Every SDK request uses the configured request timeout through the official fetch transport. At a stage deadline, the runner stops launching workflows and gives an in-flight workflow the bounded grace period to settle; transport cancellation begins if grace expires (or immediately on external/failure abort), and cleanup starts only after workers settle. Unsupported or ignored abort behavior invalidates/stops the run. Authentication performed by the measured sign-out/sign-in journey remains measured.
...
Expected denials in correctness tests pass only when the backend rejects the operation for the intended reason. Recognized measured authentication, authorization, timeout, transport/SDK, application, and backend-health errors count against the class error-rate SLO and do not by themselves invalidate a stage; malformed responses, unknown/local validation errors, external cancellation, grace expiry, and unresolved cleanup remain integrity failures. Session-loss failures retire only the affected virtual user and reduce achieved concurrency. Error examples are retained up to a configured bound so failures cannot exhaust runner memory.
