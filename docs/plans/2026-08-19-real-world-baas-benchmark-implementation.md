...
Prepare ordinary-user sessions in deterministic input order in fixed batches of ten, with no retries and no measured samples. Use `AbortController`, `performance.now()`, and promise loops. Inject `now` and `sleep` into tests. Every measured SDK request receives the configured `timeoutMs` through the official SDK fetch transport. Start the fixed-duration timer and resource collection only after every requested session is prepared; stop launching work at stage end, allow an in-flight workflow a bounded grace period, cancel its transport if grace expires (or immediately on external/failure abort), await every worker, then end measurement before unmeasured final session cleanup. An SDK that ignores the transport signal invalidates/stops rather than being silently accepted. Keep sign-out/sign-in journey authentication measured.
