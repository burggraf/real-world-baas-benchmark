...
export interface BackendInfo { name: BackendName; version: string; endpoint: string; processIds?: number[]; deviations?: string[]; }
export interface AppSession {
  ...
  createTask(input: CreateTaskInput): Promise<Task>; updateTask(input: UpdateTaskInput): Promise<Task>; addComment(input: AddCommentInput): Promise<Comment>;
  searchTasks(input: SearchTasksInput): Promise<Page<Task>>; updateProfile(input: UpdateProfileInput): Promise<User>; refreshSession(): Promise<void>; signOut(): Promise<void>; close(): Promise<void>;
}
