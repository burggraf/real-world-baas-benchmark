...
export async function collectResources(options: ResourceSamplerOptions): Promise<ResourceCollection> {
  ...
  if (options.signal?.aborted) return { samples, valid: false, validityReasons: ["aborted before sampling"] }; const event = options.eventLoop ?? (options.monitorFactory ?? monitor)();
