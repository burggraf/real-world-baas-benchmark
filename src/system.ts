...
export interface ResourceSamplerOptions { backend: BackendInfo; maxSamples?: number; runnerPid?: number; commandRunner?: CommandRunner; nowNs?: () => number; sleep?: (ms: number, signal?: AbortSignal) => Promise<void>; eventLoop?: EventLoopMonitor; intervalMs?: number; eventLoopWindowMs?: number; signal?: AbortSignal; shouldStop?: () => boolean }
...
async function sampleResourcesOnce(options: ResourceSamplerOptions): Promise<ResourceSnapshot> {
  ...
  const proc = pids.map(p => { const row = byPid.get(p); const identityOk = p === pid || (row?.command && new RegExp(info.name, "i").test(row.command)); return row && identityOk ? { pid: p, cpuPercent: row.cpuPercent, rssBytes: row.rssBytes } : { pid: p, cpuPercent: null, rssBytes: null, reason: row ? "owned PID identity mismatch" : "owned process missing or malformed" }; }); const runner = proc.find(p => p.pid === pid)!; const backendProc = proc.filter(p => p.pid !== pid); const event = options.eventLoop ?? monitor();
  if (!options.eventLoop && (options.eventLoopWindowMs ?? 10) > 0) await new Promise<void>(resolve => setTimeout(resolve, options.eventLoopWindowMs ?? 10));
...
export async function collectResources(options: ResourceSamplerOptions): Promise<ResourceCollection> {
  const max = options.maxSamples ?? 100; if (!positive(max)) throw new Error("maxSamples must be a positive safe integer"); const interval = options.intervalMs ?? 1000; if (!Number.isFinite(interval) || interval <= 0) throw new Error("intervalMs must be finite and positive"); const samples: ResourceSnapshot[] = []; const event = options.eventLoop ?? monitor(); const sleep = options.sleep ?? defaultSleep;
