...
const consistentRate = (actual: number, failed: number, attempted: number): boolean => Math.abs(actual - (attempted ? failed / attempted : 0)) <= 1e-12;
const orderedLatency = (raw: RecordValue): boolean => (raw.latencyMinMs as number) <= (raw.latencyP50Ms as number) && (raw.latencyP50Ms as number) <= (raw.latencyP95Ms as number) && (raw.latencyP95Ms as number) <= (raw.latencyP99Ms as number) && (raw.latencyP99Ms as number) <= (raw.latencyMaxMs as number);
function validateClassMetric(value: unknown, label: string): void {
  const raw = object(value, label); const attempted = integer(raw.attempted, `${label}.attempted`); const completed = integer(raw.completed, `${label}.completed`); const failed = integer(raw.failed, `${label}.failed`);
  if (attempted !== completed + failed) throw new Error(`Invalid ${label}: attempted must equal completed + failed`);
  const errorRate = finite(raw.errorRate, `${label}.errorRate`); if (errorRate > 1 || !consistentRate(errorRate, failed, attempted)) throw new Error(`Invalid ${label}.errorRate consistency`);
  for (const key of ["latencyP50Ms", "latencyP95Ms", "latencyP99Ms", "latencyMinMs", "latencyMaxMs"] as const) finite(raw[key], `${label}.${key}`);
  if (!orderedLatency(raw)) throw new Error(`Invalid ${label} latency percentile order`);
}
function validateSnapshot(value: unknown, label: string): void {
  ...
  const runner = object(raw.runner, `${label}.runner`); integer(runner.pid, `${label}.runner.pid`, 1); nullableFinite(runner.cpuPercent, `${label}.runner.cpuPercent`); if (runner.rssBytes !== null) integer(runner.rssBytes, `${label}.runner.rssBytes`); if (runner.reason !== undefined) string(runner.reason, `${label}.runner.reason`);
  const backend = object(raw.backend, `${label}.backend`); nullableFinite(backend.totalCpuPercent, `${label}.backend.totalCpuPercent`); if (backend.totalRssBytes !== null) integer(backend.totalRssBytes, `${label}.backend.totalRssBytes`); if (!Array.isArray(backend.processes)) throw new Error(`Invalid ${label}.backend.processes`); if (backend.reason !== undefined) string(backend.reason, `${label}.backend.reason`);
  for (const [index, process] of backend.processes.entries()) { const item = object(process, `${label}.backend.processes[${index}]`); integer(item.pid, `${label}.backend.processes[${index}].pid`, 1); nullableFinite(item.cpuPercent, `${label}.backend.processes[${index}].cpuPercent`); if (item.rssBytes !== null) integer(item.rssBytes, `${label}.backend.processes[${index}].rssBytes`); if (item.reason !== undefined) string(item.reason, `${label}.backend.processes[${index}].reason`); }
  if (raw.containers !== null) {
    if (!Array.isArray(raw.containers)) throw new Error(`Invalid ${label}.containers`);
    for (const [index, container] of raw.containers.entries()) { const item = object(container, `${label}.containers[${index}]`); string(item.containerId, `${label}.containers[${index}].containerId`); finite(item.cpuPercent, `${label}.containers[${index}].cpuPercent`); for (const key of ["memoryBytes", "blockReadBytes", "blockWriteBytes"] as const) integer(item[key], `${label}.containers[${index}].${key}`); }
  }
  if (raw.containerTotals !== null) { const totals = object(raw.containerTotals, `${label}.containerTotals`); nullableFinite(totals.cpuPercent, `${label}.containerTotals.cpuPercent`); for (const key of ["memoryBytes", "blockReadBytes", "blockWriteBytes"] as const) if (totals[key] !== null) integer(totals[key], `${label}.containerTotals.${key}`); }
...
function validateStage(value: unknown, index: number): void {
  ...
  const requestedUsers = integer(raw.requestedUsers, `${label}.requestedUsers`, 1); const achievedUsers = integer(raw.achievedUsers, `${label}.achievedUsers`); if (achievedUsers > requestedUsers) throw new Error(`Invalid ${label}.achievedUsers: exceeds requestedUsers`); finite(raw.elapsedSeconds, `${label}.elapsedSeconds`, Number.MIN_VALUE);
  ...
  for (const [name, operation] of Object.entries(operations)) {
    ...
    const operationCount = integer(item.operationCount, `${label}.operations.${name}.operationCount`); const errorCount = integer(item.errorCount, `${label}.operations.${name}.errorCount`); const attempted = integer(item.attemptedCount, `${label}.operations.${name}.attemptedCount`); const completed = integer(item.completedCount, `${label}.operations.${name}.completedCount`); const failed = integer(item.failedCount, `${label}.operations.${name}.failedCount`);
    if (attempted !== completed + failed || operationCount !== attempted || errorCount !== failed) throw new Error(`Invalid ${label}.operations.${name} count consistency`);
    ...
    if (!orderedLatency(item)) throw new Error(`Invalid ${label}.operations.${name} latency percentile order`);
    const errorRate = finite(item.errorRate, `${label}.operations.${name}.errorRate`); if (errorRate > 1 || !consistentRate(errorRate, failed, attempted)) throw new Error(`Invalid ${label}.operations.${name}.errorRate consistency`); if (item.successRate !== undefined) { const successRate = finite(item.successRate, `${label}.operations.${name}.successRate`); if (successRate > 1 || Math.abs(successRate - (attempted ? completed / attempted : 0)) > 1e-12) throw new Error(`Invalid ${label}.operations.${name}.successRate consistency`); }
...
export function validateBenchmarkResult(value: unknown): asserts value is BenchmarkResult {
  ...
  for (const key of ["memoryBytes", "totalMemoryBytes"] as const) if (environment[key] !== null) integer(environment[key], `environment.${key}`, 1); if (environment.logicalCores !== null) integer(environment.logicalCores, "environment.logicalCores", 1);
  for (const key of ["npmVersion", "gitCommit", "sdkVersion", "dockerVersion", "supabaseVersion"] as const) if (environment[key] !== null) string(environment[key], `environment.${key}`); if (environment.gitCommit !== null && !/^[0-9a-f]{40}$/i.test(environment.gitCommit as string)) throw new Error("Invalid environment.gitCommit");
  ...
  if (settings.sessionPreparationConcurrency !== undefined) integer(settings.sessionPreparationConcurrency, "settings.sessionPreparationConcurrency", 1);
  if (settings.executionTopology !== undefined && settings.executionTopology !== "co-located") throw new Error("Invalid settings.executionTopology");
  if (settings.capacityRefinement !== undefined) {
    const refinement = object(settings.capacityRefinement, "settings.capacityRefinement");
    if (refinement.method !== "bounded-binary-search") throw new Error("Invalid settings.capacityRefinement.method");
    integer(refinement.maxStages, "settings.capacityRefinement.maxStages", 1);
    integer(refinement.minUserGap, "settings.capacityRefinement.minUserGap", 1);
  }
  if (settings.boundarySessionsUnmeasured !== undefined && settings.boundarySessionsUnmeasured !== true) throw new Error("Invalid settings.boundarySessionsUnmeasured");
  ...
  if (correctness.aborted !== undefined) boolean(correctness.aborted, "correctness.aborted"); if (correctness.abortReason !== undefined) string(correctness.abortReason, "correctness.abortReason"); if (correctness.aborted === true && !correctness.abortReason) throw new Error("Invalid correctness abort state");
  if (!Array.isArray(raw.stages)) throw new Error("Invalid stages"); raw.stages.forEach(validateStage); const requestedStages = raw.stages.map((stage, index) => integer(object(stage, `stages[${index}]`).requestedUsers, `stages[${index}].requestedUsers`, 1)); if (new Set(requestedStages).size !== requestedStages.length || requestedStages.some((users, index) => index > 0 && users <= requestedStages[index - 1]!)) throw new Error("Invalid stage ordering or uniqueness"); if (requestedStages.some(users => users > config.maxConcurrency)) throw new Error("Invalid stage requestedUsers above maxConcurrency");
  if (!Array.isArray(raw.resources)) throw new Error("Invalid resources");
  const resourceNames = new Set<string>();
  for (const [index, resource] of raw.resources.entries()) { const item = object(resource, `resources[${index}]`); const name = string(item.name, `resources[${index}].name`); if (resourceNames.has(name)) throw new Error(`Invalid resources[${index}].name: duplicate`); resourceNames.add(name); string(item.unit, `resources[${index}].unit`); if (!Array.isArray(item.samples)) throw new Error(`Invalid resources[${index}].samples`); item.samples.forEach((sample, sampleIndex) => nullableFinite(sample, `resources[${index}].samples[${sampleIndex}]`)); if (item.reasons !== undefined) { if (!Array.isArray(item.reasons)) throw new Error(`Invalid resources[${index}].reasons`); item.reasons.forEach((reason, reasonIndex) => { if (reason !== null) string(reason, `resources[${index}].reasons[${reasonIndex}]`); }); } if (item.reason !== undefined) string(item.reason, `resources[${index}].reason`); if (item.snapshots !== undefined) { if (!Array.isArray(item.snapshots) || item.snapshots.length !== item.samples.length) throw new Error(`Invalid resources[${index}].snapshots length`); let previousTimestamp = -1; item.snapshots.forEach((snapshot, snapshotIndex) => { validateSnapshot(snapshot, `resources[${index}].snapshots[${snapshotIndex}]`); const timestamp = object(snapshot, `resources[${index}].snapshots[${snapshotIndex}]`).timestampMs as number; if (timestamp <= previousTimestamp) throw new Error(`Invalid resources[${index}] snapshot ordering`); previousTimestamp = timestamp; }); } }
  const capacity = object(raw.capacity, "capacity"); const capacityUsers = integer(capacity.users, "capacity.users"); if (capacityUsers > config.maxConcurrency) throw new Error("Invalid capacity.users above maxConcurrency"); boolean(capacity.saturation, "capacity.saturation"); strings(capacity.reasons, "capacity.reasons");
  const failures = strings(raw.failures, "failures"); const valid = boolean(raw.valid, "valid"); const validityReasons = strings(raw.validityReasons, "validityReasons"); if (valid) { if (failures.length || validityReasons.length || correctness.aborted === true || correctness.findings.some(finding => !object(finding, "correctness finding").passed) || raw.stages.length === 0 || raw.stages.some(stage => object(stage, "stage").valid !== true) || capacityUsers === 0) throw new Error("Invalid valid-result prerequisites"); const selected = raw.stages.find(stage => object(stage, "stage").requestedUsers === capacityUsers); if (!selected) throw new Error("Invalid valid-result selected capacity stage"); const selectedStage = object(selected, "selected capacity stage"); if ((selectedStage.achievedUsers as number) / (selectedStage.requestedUsers as number) < minAchievedRatio) throw new Error("Invalid selected capacity achieved ratio"); const activeClasses = new Set(configuredWorkflowNames.filter(name => config.weights[name] > 0).map(operationClassForWorkflow)); const metrics = object(selectedStage.operationClassMetrics, "selected capacity operationClassMetrics"); for (const operationClass of activeClasses) { const metric = object(metrics[operationClass], `selected capacity ${operationClass}`); const target = config.slos[operationClass]; if ((metric.attempted as number) < minClassSamples || (metric.latencyP95Ms as number) > target.p95Ms || (metric.errorRate as number) >= target.maxErrorRate) throw new Error(`Invalid selected capacity ${operationClass} SLO`); } } else if (!validityReasons.length) throw new Error("Invalid invalid-result validity reasons");
}
...
const freeText = (value: string): string => value
  ...
  .replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/g, " ")
  .slice(0, 300);
const markdownText = (value: unknown): string => freeText(String(value)).replace(/\\/g, "\\\\").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
const mdCell = (value: unknown): string => markdownText(value).replace(/[|\[\]`*_]/g, character => `\\${character}`).replace(/\r?\n/g, "<br>");
const mdLinkText = (value: unknown): string => markdownText(value).replace(/[\[\]]/g, character => `\\${character}`).replace(/\r?\n/g, " ");
...
const csvField = (value: unknown): string => { let text = freeText(String(value)); if (/^\s*[=+\-@]/.test(text)) text = `'${text}`; return /[",\r\n]/.test(text) ? `"${text.replace(/"/g, '""')}"` : text; };
...
export function createBenchmarkReport(result: BenchmarkResult, rawJsonPath: string): BenchmarkReport {
  ...
  const markdown = [
    ...
    "## Run", "", markdownTable(["Field", "Value"], [["Backend", result.backend.name], ["Backend version", result.backend.version], ["Config", result.config.name], ["Dataset", result.dataset], ["Seed", result.seed], ["Publishable", result.publishable ? "yes" : "no"], ["Execution topology", result.settings.executionTopology ?? "unclassified legacy result"], ["Run ID", result.runId], ["Started at", result.startedAt]]), "",
