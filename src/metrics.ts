...
const safeClassifications = new Set(["authentication", "authorization", "timeout", "transport/sdk", "invalid_response", "backend_health", "application"]);
const validStatus = (status: unknown): boolean => status === undefined || (Number.isInteger(status) && (status as number) >= 100 && (status as number) <= 599);
...
export class StageMetricsAccumulator {
  ...
  record(sample: WorkloadSample): void {
    ...
    if (sample.success) b.completed++; else { b.failed++; const c=classify(sample.error!); b.errors.set(c,(b.errors.get(c)??0)+1); const safeName=redact(sample.error!.name).slice(0,100); const safeMessage=redact(sample.error!.message); const ek=`${k}|${c}|${sample.error!.code ?? ""}|${sample.error!.status ?? ""}|${safeMessage}`; if (!this.examples.has(ek) && this.examples.size < this.maxExamples) this.examples.set(ek,{type:sample.type,name:sample.name,workflow:sample.workflow,operationClass:sample.operationClass,kind:sample.kind,classification:c,nameOfError:safeName,message:safeMessage,occurrences:1,...sample.error!.code ? { code: sample.error!.code } : {}, ...sample.error!.status !== undefined ? { status: sample.error!.status } : {}}); else if (this.examples.has(ek)) this.examples.get(ek)!.occurrences++; }
  }
  finalize(elapsedSeconds: number, users: { requestedUsers: number; achievedUsers: number }): StageMetrics {
    ...
    const classAggregates = new Map<OperationClass, { attempted: number; completed: number; failed: number; latencies: number[] }>(classes.map(name => [name, { attempted: 0, completed: 0, failed: 0, latencies: [] }]));
    for(const [k,b] of this.buckets){ const l=[...b.latencies].sort((a,c)=>a-c); const pct=(p:number)=>l.length?l[Math.min(l.length-1,Math.ceil(p*l.length)-1)]!:0; const metric:OperationMetric={type:b.sample.type,name:b.sample.name,workflow:b.sample.workflow,operationClass:b.sample.operationClass,kind:b.sample.kind,attemptedCount:b.attempted,completedCount:b.completed,failedCount:b.failed,errorRate:finiteDivide(b.failed,b.attempted,"operation errorRate"),successRate:finiteDivide(b.completed,b.attempted,"operation successRate"),throughputPerSecond:finiteDivide(b.completed,elapsedSeconds,"operation throughput"),operationCount:b.attempted,errorCount:b.failed,latencyP50Ms:pct(.5),latencyP95Ms:pct(.95),latencyP99Ms:pct(.99),latencyMinMs:l[0]??0,latencyMaxMs:l[l.length-1]??0,errorCounts:Object.fromEntries(b.errors)}; operations[k]=metric; if(b.sample.type==="workflow"){workflowDone+=b.completed;workflowCounts[b.sample.name]=(workflowCounts[b.sample.name]??0)+b.completed; const aggregate=classAggregates.get(b.sample.operationClass)!; aggregate.attempted+=b.attempted; aggregate.completed+=b.completed; aggregate.failed+=b.failed; aggregate.latencies.push(...b.latencies);} else {sdkDone+=b.completed;if(b.sample.kind==="read")reads+=b.completed;else writes+=b.completed;} }
    for(const [n,c] of Object.entries(workflowCounts)) workflowByName[n]=finiteDivide(c,elapsedSeconds,"workflow throughput");
    const operationClassMetrics = Object.fromEntries(classes.map(name => { const aggregate=classAggregates.get(name)!; const l=[...aggregate.latencies].sort((a,c)=>a-c); const pct=(p:number)=>l.length?l[Math.min(l.length-1,Math.ceil(p*l.length)-1)]!:0; return [name, { attempted: aggregate.attempted, completed: aggregate.completed, failed: aggregate.failed, errorRate: finiteDivide(aggregate.failed,aggregate.attempted,"operation class errorRate"), latencyP50Ms:pct(.5), latencyP95Ms:pct(.95), latencyP99Ms:pct(.99), latencyMinMs:l[0]??0, latencyMaxMs:l[l.length-1]??0 }]; })) as Record<OperationClass, OperationClassMetric>;
    return {requestedUsers:users.requestedUsers,achievedUsers:users.achievedUsers,elapsedSeconds,workflowTransactionsPerSecond:finiteDivide(workflowDone,elapsedSeconds,"workflow aggregate throughput"),workflowTransactionsPerSecondByName:workflowByName,sdkOperationsPerSecond:finiteDivide(sdkDone,elapsedSeconds,"SDK aggregate throughput"),readOperationsPerSecond:finiteDivide(reads,elapsedSeconds,"read throughput"),writeOperationsPerSecond:finiteDivide(writes,elapsedSeconds,"write throughput"),workflowCompletionCountByName:workflowCounts,operationClassMetrics,operations,errorExamples:[...this.examples].map(([,e])=>({...e})),valid:!this.invalidReason,validityReasons:this.invalidReason?[this.invalidReason]:[]};
  }
}
