...
const rate = (count: number, elapsedSeconds: number): number => { if (count === 0) return 0; const value = count / elapsedSeconds; if (!Number.isFinite(value)) throw new Error("elapsedSeconds cannot produce finite metrics"); return value; };
