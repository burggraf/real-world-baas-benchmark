...
export class StageMetricsAccumulator {
  ...
  finalize(elapsedSeconds: number): StageMetrics {
    if (this.finalized) throw new Error("Stage already finalized"); if (!Number.isFinite(elapsedSeconds) || elapsedSeconds <= 0) throw new Error("elapsedSeconds must be finite and positive"); if (!Number.isFinite(1 / elapsedSeconds)) throw new Error("elapsedSeconds cannot produce finite metrics"); this.finalized=true;
